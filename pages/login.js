...
const login = () => {
  useEffect(() => {
    ...
    form.addEventListener("submit", (e) => {
      e.preventDefault();
      const admin = fetch(
        `http://localhost:3000/admin?email=${email.value}&password=${pass.value}`
      )
        .then((res) => res.json())
        .then((user) => router.navigate("/admin/projects"));
    });
  });
  return /*html*/ `
                ...
                <div class="mt-4">
                    <label class="block">Password<label>
                            <input type="password" placeholder="Password"
                                class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600" id='pass'>
                </div>
